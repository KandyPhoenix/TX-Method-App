...
function lineChart(canvas, series, labels) {
  ...
  ctx.strokeStyle = '#2a2a2a'; ctx.fillStyle = '#7a9650'; ctx.font = '10px -apple-system,sans-serif'; ctx.lineWidth = 1;
  for (let g = 0; g <= 4; g++) {
  ...
  series.forEach(s => {
    ...
    ctx.fillStyle = '#7a9650'; ctx.fillText(s.name, lx + 14, 8);
...
function toast(msg) {
  ...
  if (!t) {
    ...
    Object.assign(t.style, { position:'fixed', bottom:'92px', left:'50%', transform:'translateX(-50%)',
      background:'#1e1e1e', color:'#f0f9e8', padding:'12px 18px', borderRadius:'12px', zIndex:60,
      border:'1px solid #2a2a2a', fontWeight:'700', boxShadow:'0 10px 30px rgba(0,0,0,.6)', transition:'opacity .3s' });
  }
