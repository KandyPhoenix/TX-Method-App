...
function lineChart(canvas, series, labels) {
  ...
  ctx.strokeStyle = '#2c2c2c'; ctx.fillStyle = '#888888'; ctx.font = '10px -apple-system,sans-serif'; ctx.lineWidth = 1;
  for (let g = 0; g <= 4; g++) {
  ...
  series.forEach(s => {
    ...
    ctx.fillStyle = '#888888'; ctx.fillText(s.name, lx + 14, 8);
...
function toast(msg) {
  ...
  if (!t) {
    ...
    Object.assign(t.style, { position:'fixed', bottom:'92px', left:'50%', transform:'translateX(-50%)',
      background:'#1e1e1e', color:'#f5f5f5', padding:'12px 18px', borderRadius:'12px', zIndex:60,
      border:'1px solid #2c2c2c', fontWeight:'700', boxShadow:'0 10px 30px rgba(0,0,0,.6)', transition:'opacity .3s' });
  }
